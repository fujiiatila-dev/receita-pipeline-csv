...
if len(sys.argv) < 3:
    print("Uso: process_empresas.py <mes_ano> <tipo_arquivo>")
    sys.exit(1)

# args: 202505 Empresas
MES_ANO = sys.argv[1]
TYPE = sys.argv[2] 
...
# Lê todos os arquivos que comecem com o Tipo (ex: Empresas0.csv, Empresas1.csv...)
...
try:
    ...
    # Renomeia o arquivo part-00000... para o nome final
    part_files = glob.glob(f"{TEMP_OUTPUT_DIR}/part-*.csv")
    if part_files:
        os.makedirs(os.path.dirname(FINAL_FILE), exist_ok=True)
        shutil.move(part_files[0], FINAL_FILE)
        print(f"Arquivo final gerado com sucesso: {FINAL_FILE}")
    else:
        raise Exception("Arquivo de parte não encontrado após gravação.")
